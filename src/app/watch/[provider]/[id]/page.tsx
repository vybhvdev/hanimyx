...
export default async function WatchPage({ 
  ...
  // Get info which now includes the manifest already
  ...
  const streams = await hanime.getStreams(videoInfo);
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm font-bold">{videoData.views.toLocaleString()}</p>
              </div>
              <div>
                <p className="text-[10px] text-white/30 uppercase">Rating</p>
                <p className="text-sm font-bold">{videoData.rating}/10</p>
              </div>
              <div>
                <p className="text-[10px] text-white/30 uppercase">Likes</p>
                <p className="text-sm font-bold">{videoData.likes.toLocaleString()}</p>
              </div>
              <div>
                <p className="text-[10px] text-white/30 uppercase">Downloads</p>
                <p className="text-sm font-bold">{videoData.downloads.toLocaleString()}</p>
              </div>
            </div>
          </div>
